...
% x = rand(size(x));
L = discretize(x, 20); %This simplifies the real numbers. Categorizes the data. Takes the decimals and rounds up to a whole number
...
p.EdgeCData = T(~~T(:));
% p.EdgeCData = randi([1 10], sum(~~T(:)), 1); %colors the edges of the graph. If you sum these 1s, they will tell you the value of non-zero elements
% p.NodeCData = randi([1 10], 20, 1); %create a new graph with the data 
colorbar

p.LineWidth = rescale(T(~~T(:)), 1, 5); %Adds a thickness to the line and makes it proportional 
